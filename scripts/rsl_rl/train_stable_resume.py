...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Name of the task. Known Unitree tasks are discovered dynamically at runtime.",
)
parser.add_argument("--seed", type=int, default=None, help="Seed used for the environment")
parser.add_argument("--max_iterations", type=int, default=None, help="RL Policy training iterations.")
parser.add_argument(
    "--resume_reset_optimizer_state",
    action="store_true",
    default=True,
    help="When resuming, clear optimizer state for conservative fine-tuning.",
)
parser.add_argument(
    "--resume_keep_optimizer_state",
    action="store_true",
    default=False,
    help="When resuming, keep the checkpoint optimizer state instead of clearing it.",
)
parser.add_argument(
    "--resume_override_learning_rate",
    type=float,
    default=None,
    help="When resuming, override optimizer lr. Defaults to agent_cfg.algorithm.learning_rate.",
)
parser.add_argument(
    "--resume_reset_std_to",
    type=float,
    default=0.2,
    help="When resuming, reset action std to this value before conservative fine-tuning.",
)
parser.add_argument(
    "--resume_std_min",
    type=float,
    default=0.05,
    help="Lower clamp for action std in conservative resume mode.",
)
parser.add_argument(
    "--resume_std_max",
    type=float,
    default=0.5,
    help="Upper clamp for action std in conservative resume mode.",
)
parser.add_argument(
    "--resume_freeze_std",
    action="store_true",
...
def apply_conservative_resume_overrides(
    runner: OnPolicyRunner,
    learning_rate: float,
    reset_optimizer_state: bool,
    reset_std_to: float | None,
    std_min: float,
    std_max: float,
    freeze_std: bool,
):
    ...
    if optimizer is not None:
        if reset_optimizer_state:
            ...
            print("[INFO]: Cleared optimizer state for conservative resume.", flush=True)
        for param_group in optimizer.param_groups:
            param_group["lr"] = learning_rate
        print(f"[INFO]: Overrode optimizer learning rate to {learning_rate}.", flush=True)

    policy_nn = getattr(runner.alg, "actor_critic", None)
    if policy_nn is None:
        policy_nn = getattr(runner.alg, "policy", None)
    if policy_nn is not None and hasattr(policy_nn, "std"):
        with torch.no_grad():
            if reset_std_to is not None:
                policy_nn.std.fill_(float(reset_std_to))
            policy_nn.std.data.clamp_(min=std_min, max=std_max)
        print(
            "[INFO]: Conservative resume action std setup "
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if agent_cfg.resume or agent_cfg.algorithm.class_name == "Distillation":
        ...
        conservative_resume_reset_optimizer_state = (
            args_cli.resume_reset_optimizer_state and not args_cli.resume_keep_optimizer_state
        )
        conservative_resume_learning_rate = (
            float(args_cli.resume_override_learning_rate)
            if args_cli.resume_override_learning_rate is not None
            else float(agent_cfg.algorithm.learning_rate)
        )
        apply_conservative_resume_overrides(
            runner,
            learning_rate=conservative_resume_learning_rate,
            reset_optimizer_state=conservative_resume_reset_optimizer_state,
