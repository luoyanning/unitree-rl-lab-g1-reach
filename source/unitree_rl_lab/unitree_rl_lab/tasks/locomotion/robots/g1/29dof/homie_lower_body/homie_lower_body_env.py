...
class G1HomieLowerBodyEnv(DirectRLEnv):
    ...
    def _compute_intermediate_values(self):
        ...
        contact_mask = self._current_contacts.float()
        pseudo_force_z = (50.0 + 200.0 * torch.clamp(-self._feet_vel_w[:, :, 2], min=0.0)) * contact_mask
        pseudo_force_x = -25.0 * self._feet_vel_w[:, :, 0] * contact_mask
        pseudo_force_y = -25.0 * self._feet_vel_w[:, :, 1] * contact_mask
        self._foot_contact_forces = torch.stack((pseudo_force_x, pseudo_force_y, pseudo_force_z), dim=-1)
