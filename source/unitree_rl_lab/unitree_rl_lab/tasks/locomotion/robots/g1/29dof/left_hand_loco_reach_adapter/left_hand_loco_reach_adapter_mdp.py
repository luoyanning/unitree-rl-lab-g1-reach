...
def _compute_adapter_command(
    ...
    blend = torch.where(near_success, torch.ones_like(blend), blend)
    ...
    adapter_command[near_success] = torch.stack(
        (
            torch.clamp(target_pos_base[near_success, 0], min=x_cmd_range[0], max=x_cmd_range[1]),
            torch.clamp(target_pos_base[near_success, 1], min=y_cmd_range[0], max=y_cmd_range[1]),
            torch.clamp(target_pos_base[near_success, 2], min=z_cmd_range[0], max=z_cmd_range[1]),
        ),
        dim=-1,
    )
