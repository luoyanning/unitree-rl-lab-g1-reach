...
SETTLE_GRACE_S = 1.5
FREEZE_BASE_READY_THRESHOLD = 0.50
...
def _sync_adapter_hold_stay_state(
    ...
    settle_grace_steps = max(0, int(round(SETTLE_GRACE_S / env.step_dt)))
    ...
    if fixed_mdp.ENABLE_TARGET_TIMEOUT_DEBUG and not env._left_hand_timeout_cfg_logged:
        print(
            ...
            f"settle_grace_s={float(SETTLE_GRACE_S):.3f} "
            f"settle_grace_steps={int(settle_grace_steps)} "
    ...
    env._left_hand_post_success_dwell_counter = torch.where(
        ...
        torch.zeros_like(env._left_hand_post_success_dwell_counter),
    )
    env._left_hand_in_post_success_dwell[:] = dwell_in_zone

    dwell_done = dwell_in_zone & (env._left_hand_post_success_dwell_counter >= dwell_steps)
    ...
    settle_grace_active = env._left_hand_success_zone_time > 0
    effective_timeout_steps = per_target_timeout_steps + settle_grace_steps * settle_grace_active.long()
    pre_timeout = env._left_hand_target_age_steps >= effective_timeout_steps
    ...
    post_timeout = env._left_hand_target_age_steps >= effective_timeout_steps
    ...
    if fixed_mdp.ENABLE_LONG_HORIZON_DEBUG_METRICS and hasattr(command_term, "metrics"):
        ...
        command_term.metrics.setdefault("effective_target_timeout_steps", torch.zeros(num_envs, device=env.device))
        command_term.metrics["effective_target_timeout_steps"][:] = effective_timeout_steps.float()
        command_term.metrics.setdefault("settle_grace_active_flag", torch.zeros(num_envs, device=env.device))
        command_term.metrics["settle_grace_active_flag"][:] = settle_grace_active.float()
    ...
    if base_velocity_command is not None:
        freeze_base_mask = _freeze_base_reach_mask(
            env,
            command_name=command_name,
            x_range=x_range,
            y_range=y_range,
    ...
    fixed_mdp._debug_timeout_state(
        ...
        max_target_steps=per_target_timeout_steps + settle_grace_steps,
    )
...
def target_timeout_reached(
    ...
    per_target_timeout_steps = max(1, int(round(per_target_timeout_s / env.step_dt)))
    settle_grace_steps = max(0, int(round(SETTLE_GRACE_S / env.step_dt)))
    effective_timeout_steps = per_target_timeout_steps + settle_grace_steps * (env._left_hand_success_zone_time > 0).long()
    return env._left_hand_target_age_steps >= effective_timeout_steps
...
def post_success_stay_reward(
    ...
    position_error = fixed_mdp._static_target_position_error(env, asset_cfg=asset_cfg)
    stay_gate = env._left_hand_in_post_success_dwell.float()
    stability = torch.exp(-ee_speed / hand_speed_scale) * torch.exp(-base_speed / base_speed_scale)
    return stay_gate * torch.exp(-position_error / stay_reward_std) * stability


def _ready_reach_gate(
    env,
    command_name: str,
    x_range: tuple[float, float],
    y_range: tuple[float, float],
    ...
    ready_gate = fixed_mdp._workspace_ready_gate(
        env,
        command_name=command_name,
        x_range=x_range,
        y_range=y_range,
    ...
    switch_phase = fixed_mdp._switch_phase_scale(env, switch_phase_steps)
    active_phase = (~env._left_hand_in_success_zone & ~env._left_hand_in_post_success_dwell).float()
    return ready_gate * (1.0 - switch_phase) * active_phase


def _freeze_base_reach_mask(
    env,
    command_name: str,
    x_range: tuple[float, float],
    y_range: tuple[float, float],
    switch_phase_steps: int,
    gate_std: float,
    ready_threshold: float = FREEZE_BASE_READY_THRESHOLD,
):
    ready_gate = fixed_mdp._workspace_ready_gate(
        env,
        command_name=command_name,
    ...
    switch_phase = fixed_mdp._switch_phase_scale(env, switch_phase_steps)
    active_phase = ~env._left_hand_in_success_zone & ~env._left_hand_in_post_success_dwell
    return active_phase & (ready_gate >= ready_threshold) & (switch_phase <= 0.50)


def ready_reach_right_arm_neutral_reward(
    env,
    asset_cfg: SceneEntityCfg,
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    return reach_gate * torch.exp(-joint_dev / joint_dev_scale)


def ready_reach_stationary_reward(
    env,
    asset_cfg: SceneEntityCfg,
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    return reach_gate * stability


def ready_reach_left_hand_stillness_reward(
    env,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", body_names=["left_wrist_yaw_link"]),
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    return reach_gate * torch.exp(-ee_speed / hand_speed_scale)


def ready_reach_left_hand_vertical_motion_penalty(
    env,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", body_names=["left_wrist_yaw_link"]),
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    sample_weights: dict[str, float] | None = None,
    gate_std: float = 0.01,
    vertical_speed_scale: float = 0.05,
):
    _sync_adapter_hold_stay_state(
        env,
        command_name=command_name,
    ...
    if ee_lin_vel.ndim == 3:
        ee_lin_vel = ee_lin_vel[:, 0]
    z_speed = torch.abs(ee_lin_vel[:, 2])
    reach_gate = _ready_reach_gate(
        env,
        command_name=command_name,
        x_range=x_range,
    ...
    return reach_gate * torch.tanh(z_speed / vertical_speed_scale)


def ready_reach_left_arm_joint_acc_penalty(
    env,
    asset_cfg: SceneEntityCfg,
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    acc_mag = torch.linalg.norm(joint_acc, dim=1)
    reach_gate = _ready_reach_gate(
        env,
        command_name=command_name,
        x_range=x_range,
    ...
    return reach_gate * torch.tanh(acc_mag / joint_acc_scale)


def ready_reach_foot_shuffle_penalty(
    env,
    asset_cfg: SceneEntityCfg,
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    return reach_gate * torch.tanh(foot_speed / foot_speed_scale)
...
def near_target_left_hand_stillness_reward(
    ...
    ee_speed = torch.linalg.norm(ee_lin_vel, dim=-1)
    position_error = fixed_mdp._static_target_position_error(env, asset_cfg=asset_cfg)
    near_gate = ((position_error <= near_target_radius) & ~env._left_hand_in_post_success_dwell).float()
    return near_gate * torch.exp(-ee_speed / hand_speed_scale)


def dwell_left_hand_stillness_reward(
    env,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", body_names=["left_wrist_yaw_link"]),
    command_name: str = "left_hand_pose",
    success_threshold: float = 0.06,
    ...
    dwell_steps = max(1, int(post_success_dwell_steps))
    dwell_progress = torch.clamp(env._left_hand_post_success_dwell_counter.float() / float(dwell_steps), min=0.0, max=1.0)
    return dwell_gate * torch.exp(-ee_speed / hand_speed_scale) * dwell_progress
