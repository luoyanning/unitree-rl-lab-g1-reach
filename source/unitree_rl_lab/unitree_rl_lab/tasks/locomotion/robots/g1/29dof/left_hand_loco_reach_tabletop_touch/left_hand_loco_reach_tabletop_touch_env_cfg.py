...
def _update_term_params(term_cfg, **updates) -> None:
    params = getattr(term_cfg, "params", None)
    if not isinstance(params, dict):
        return
...
class RobotLeftHandLocoReachTableTopTouchScratchEnvCfg(RobotLeftHandLocoReachTableTopTouchEnvCfg):
    ...
    def __post_init__(self):
        ...
        for term_cfg in (
            ...
            _update_term_params(term_cfg, **scratch_params)
        ...
        for reward_name in (
            ...
            _update_term_params(getattr(self.rewards, reward_name), **scratch_params)
...
@configclass
class RobotLeftHandLocoReachTableTopTouchScratchPlayEnvCfg(RobotLeftHandLocoReachTableTopTouchScratchEnvCfg):
    def __post_init__(self):
        super().__post_init__()
