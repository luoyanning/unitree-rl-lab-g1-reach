...
class RearTurnPointGoalCommand(PointGoalCommand):
    ...
    def _update_guidance_command(self):
        ...
        lin_vel_x = torch.where(hold_mode | recovery_turn_mode | turn_in_place_mode, torch.zeros_like(lin_vel_x), lin_vel_x)
        lin_vel_y = torch.where(hold_mode | recovery_turn_mode | turn_in_place_mode, torch.zeros_like(lin_vel_y), lin_vel_y)
